    ...
    if (!str) {
      if (!expSlots.length && strings.length === 1 && strings[0] === "") {
        ...
        str = strings.reduce(function interlaceTemplate(html, strVal, i) {
          html += strVal
          return expSlots[i] !== undefined ? addExpressions(expSlots[i], html) : html
        }, "")
      }
    }
    return str
...
function removeNode(node) {
  var _a
  node.remove()
  ;(_a = listeners.get(node)) === null || _a === void 0
    ? void 0
    : _a.forEach((listener, event) => node.removeEventListener(event, listener))
}
/**
...
function setNode(value, p) {
  ...
  Array.isArray(value) ? value.forEach((item) => partial.add(item)) : partial.add(value)
