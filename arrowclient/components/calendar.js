...
function nextMonth() {
	const nextMonthDate = dayjs(selectedDate.date)
		.add(1, "month")
		.format("YYYY-MM-DD");
...
function prevMonth() {
	const prevMonthDate = dayjs(selectedDate.date)
		.subtract(1, "month")
		.format("YYYY-MM-DD");
...
function nextYear() {
	const nextYearDate = dayjs(selectedDate.date)
		.add(1, "year")
		.format("YYYY-MM-DD");
...
function prevYear() {
	const prevYearDate = dayjs(selectedDate.date)
		.subtract(1, "year")
		.format("YYYY-MM-DD");
...
export default () => {
	...
	let week = (date) => {
		...
		const focused = isSelectedDate
			? "dark:border-mygray border-mygray "
			: "border-transparent";
		...
		if (date)
			...
			>
				<div
					class="notomono leading-6 w-full h-full border-2 ${focused} ${calendarDot()} ${today}"
				>
					${date.date()}
				</div>
			</td>`;
	...
	return html`
		<div
			class="notomono w-full text-center p-3 bg-nearwhite dark:text-white dark:bg-black"
		>
