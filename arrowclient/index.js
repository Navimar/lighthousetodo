...
let renderTasks = () => {
  ...
  if (data.visibletasks[0] && (dayjs(data.visibletasks[0].time, "HH:mm").isAfter(dayjs()) || data.visibletasks[0].pause)) {

    let swapedtasks = data.visibletasks.slice();
    // Find the index of the first task that's due or overdue based on the current time
    let index = swapedtasks.findIndex(task => dayjs(task.time + ' ' + task.date, "HH:mm YYYY-MM-DD").isSameOrBefore(dayjs()) && !task.pause);
