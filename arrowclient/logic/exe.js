...
export const makevisible = () => {
  data.visibletasks = data.tasks.filter(task => {
    ...
    const isCurrentOrFutureTask = selectedDate.date === currentTime.date
    ...
    return isCurrentOrFutureTask && (task.fromNames.length === 0 || task.type === 'meeting');
  });
...
let saveTask = (m) => {
  ...
  if (eDiv) {
    ...
    const fromEditLines = [...new Set(fromEdit.innerText.trim().split("\n").filter(line => line.trim() !== ''))];
    const toEdit = document.getElementById("toEdit");
    const toEditLines = [...new Set(toEdit.innerText.trim().split("\n").filter(line => line.trim() !== ''))];
