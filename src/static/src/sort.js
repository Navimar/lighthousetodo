let sortdata = (arr = data.tasks) => {
	...
	arr.sort((a, b) => {
		...
		if (parseInt(adip) > parseInt(bdip)) return 1;
		if (parseInt(bdip) > parseInt(adip)) return -1;

		if (
			moment() <= moment(a.date + "T" + a.time) &&
			moment() > moment(b.date + "T" + b.time)
		)
			return 1;
		if (
			moment() > moment(a.date + "T" + a.time) &&
		...
		if (
			...
			if (moment(a.date + "T" + a.time) > moment(b.date + "T" + b.time)) {
				...
				moment(a.date + "T" + a.time) < moment(b.date + "T" + b.time)
			) {
				return -1;
			}
		}
		...
		if (
			a.linksfromNames[0] &&
			b.linksfromNames[0] &&
			a.linksfromNames[0].localeCompare(b.linksfromNames[0]) < 0
		) {
			return -1;
		} else if (
			a.linksfromNames[0] &&
...
let trans = (x) => {
	...
	if (x == "fifth") return 5;
	if (x == "sixth") return 6;
	if (x == "seventh") return 7;
	if (x == "eighth") return 8;
	if (x == "ninth") return 9;
	if (x == "tenth") return 10;
	if (x == "eleventh") return 11;
};
...
let retrans = (x) => {
	...
	if (x == 9) return "ninth";
	if (x == 10) return "tenth";
	if (x == 11) return "eleventh";
};
