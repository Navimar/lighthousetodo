...
function convertNumToText(num) {
    ...
    const tens_special = ['десять', 'одиннадцать', 'двенадцать', 'тринадцать', 'четырнадцать', 'пятнадцать', 'шестнадцать', 'семнадцать', 'восемнадцать', 'девятнадцать'];
    ...
    if (num === 0) {
        ...
        if (num < 100) {
            if (num < 10) {
                ...
                return tens_special[num - 10]
            }
            else {
                return tens[num[0]] + ' ' + ones[num[1]];
            }
        } else {
            return hundreds[num[0]] + ' ' + tens[num[1]] + ' ' + ones[num[2]];
        }
    }
}